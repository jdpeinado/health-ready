...
**Architecture:** Vite + React + TypeScript SPA. React Router for navigation, TanStack Query for server-state/caching, a thin `fetch` API client that sends the session cookie (`credentials: "include"`). Recharts for progress charts. `vite-plugin-pwa` makes it installable. The API gains CORS-with-credentials so the browser may call it cross-origin. **Styling is Tailwind CSS v4 + shadcn/ui**: shadcn components are generated into `src/components/ui/` (we own the code, MIT) and composed with Tailwind utility classes for a uniform, accessible, mobile-first look.

**Tech Stack:** Vite, React 18, TypeScript, react-router-dom 6, @tanstack/react-query 5, recharts, vite-plugin-pwa, **Tailwind CSS v4 (`@tailwindcss/vite`) + shadcn/ui (Radix primitives, lucide-react icons, `class-variance-authority`, `clsx`, `tailwind-merge`)**; Vitest + @testing-library/react + jsdom for tests.
...
These conventions are used by every screen task below. The shadcn primitives are generated once in Task 2 (`button`, `input`, `label`, `card`, `select`, `textarea`) into `src/components/ui/` and imported via the `@/` path alias.

- **Imports:** components from `@/components/ui/<name>`; the `cn()` class-merge helper from `@/lib/utils`.
- **Page container:** the authenticated `Layout` wraps content in `<main className="mx-auto max-w-screen-sm px-4 pb-24 pt-4">` (the `pb-24` clears the fixed bottom nav). Each page returns a `<div className="space-y-4">` root.
- **Cards:** `Card` / `CardHeader` / `CardTitle` / `CardContent` from `@/components/ui/card` replace the old `.card` class.
- **Buttons:** `Button` from `@/components/ui/button`. Variants used: default (primary action), `secondary`, `destructive`, `outline`, `ghost`. Use `size="sm"` for inline actions and `className="w-full"` for full-width submit buttons.
- **Form fields:** wrap each in `<div className="space-y-1.5">` with a `Label` + the control (`Input`, `Textarea`, or `Select`). Two fields side-by-side: `<div className="grid grid-cols-2 gap-3">`.
- **Selects:** shadcn `Select` is composite — `Select` (`value` + `onValueChange`) › `SelectTrigger` › `SelectValue` › `SelectContent` › `SelectItem value=…`. Native `<select>` is **not** used.
- **Text helpers:** muted text → `className="text-muted-foreground text-sm"`; error text → `className="text-destructive text-sm"`; headings → `text-xl font-semibold` (h3) / `text-2xl font-bold` (h1).
- **Theme:** dark by default — `index.html` sets `<html lang="es" class="dark">`. shadcn is initialized with the **slate** base color and CSS variables, which preserves the app's existing dark slate palette. Chart colors read from CSS variables where practical.
...
## Task 2: Scaffold the web app (Vite + React + PWA + Tailwind/shadcn)
...
- Generated by shadcn init/add: `apps/web/components.json`, `apps/web/src/lib/utils.ts`, `apps/web/src/components/ui/*.tsx`
...
> `class-variance-authority`, `clsx`, `tailwind-merge`, and `lucide-react` are what shadcn components import. They're listed up front so `pnpm install` resolves them; `shadcn init`/`add` will also add any `@radix-ui/*` packages each component needs (e.g. `@radix-ui/react-select`, `@radix-ui/react-label`, `@radix-ui/react-slot`).
...
> `baseUrl` + `paths` give us the `@/*` → `./src/*` alias that shadcn components import with. `shadcn init` reads this to confirm the alias. (We use a single `tsconfig.json` per package — there's no `tsconfig.app.json`/`tsconfig.node.json` split, so the alias lives here.)
...
Tailwind v4 needs only the single import to start. `shadcn init` (Step 9) will append the theme tokens (the `:root`/`.dark` CSS variables + `@theme inline` + the `@custom-variant dark` line that makes `class="dark"` work).
...
ReactDOM.createRoot(document.getElementById("root")!).render(
  <React.StrictMode>
    <main className="mx-auto max-w-screen-sm p-4">
      <h1 className="text-2xl font-bold">Health Ready</h1>
    </main>
  </React.StrictMode>,
);
```
...
Run: `pnpm install`
Expected: installs web deps (React, Tailwind, testing libs, etc.).

- [ ] **Step 9: Initialize shadcn/ui and add the base components**

From `apps/web`, initialize shadcn (Tailwind v4 + CSS variables, **slate** base color):

```bash
pnpm dlx shadcn@latest init --base-color slate
```

> Non-interactive where possible. If the CLI still prompts, accept: base color **Slate**, CSS variables **yes**. It writes `components.json`, creates `src/lib/utils.ts` (the `cn()` helper), augments `src/index.css` with the theme tokens, and installs `class-variance-authority`/`clsx`/`tailwind-merge`/`lucide-react` (already in `package.json`). It reads the `@/*` alias from `tsconfig.json` + `vite.config.ts`.

Then add the components every screen uses:

```bash
pnpm dlx shadcn@latest add button input label card select textarea
```

This generates `src/components/ui/{button,input,label,card,select,textarea}.tsx` and adds the matching `@radix-ui/*` deps. Confirm the files exist before moving on.

- [ ] **Step 10: Add PWA icon placeholders**
...
Expected: Vite serves on `http://localhost:5173` showing the "Health Ready" heading styled by Tailwind (dark background). Stop the server.

- [ ] **Step 12: Checkpoint**

✋ Good point to commit — **the user handles all git staging/commits**. Suggested message: `feat(web): scaffold vite react PWA shell with tailwind + shadcn/ui`
        ...
        {w.entries.map((e) => (
          <Card key={e.id}>
            <CardContent className="space-y-1 py-3">
              <strong className="font-medium">Ejercicio</strong>
              {e.comment && (
...
> Note: the entry shows the placeholder `Ejercicio` for the exercise name; Task 8 replaces
> it with a real name lookup from the exercise library.
...
Then replace the placeholder `<strong className="font-medium">Ejercicio</strong>` line with:
...
Expected: `tsc --noEmit` clean and Vite emits `apps/web/dist` (Tailwind CSS compiled, plus a generated service worker + manifest from `vite-plugin-pwa`).
